...
public class ClimberSubsystem extends SubsystemBase {
        ...
        switch (state) {
            ...
            case HOMED:
                ...
                if (RobotContainer.driveHID.getLeftBumper()
                        || RobotContainer.driveHID.getRightBumper()
                        || RobotContainer.driveHID.getLeftTriggerAxis() > 0.1
                        || RobotContainer.driveHID.getRightTriggerAxis() > 0.1) {
    ...
    private boolean controlMotors() {
        double leftSpeed = 0;
        double rightSpeed = 0;
        if (RobotContainer.driveHID.getLeftTriggerAxis() > 0.1) {
            leftSpeed = -SPEED; // Move climber down
        }
        if (RobotContainer.driveHID.getRightTriggerAxis() > 0.1) {
            rightSpeed = SPEED; // Move climber down
        }
        if (RobotContainer.driveHID.getLeftBumper()) {
            leftSpeed = SPEED; // Move climber up
        }
        if (RobotContainer.driveHID.getRightBumper()) {
            rightSpeed = -SPEED; // Move climber up
        }
        // TODO Uncomment
        // if (Math.abs(getEncoderPosition(leftClimber)) > 80 && RobotContainer.driveHID.getLeftBumper()) {
        //     leftSpeed = 0;

        // }
        // if (Math.abs(getEncoderPosition(rightClimber)) > 80 && RobotContainer.driveHID.getRightBumper()) {
        //     rightSpeed = 0;
        // }
